...
param(
    [string]$InstallDir   = "$env:ProgramFiles\socks5-ss-proxy",
...
# --- Step 2: Download binary from latest GitHub Actions run ---
...
if ($ghAvailable) {
    # Use gh to download artifact from latest successful run
...
if (-not $ghAvailable) {
    # Fallback: download from latest release or build manually
...
'@
    ...
    notepad.exe $keysDestPath
    Read-Host "Press Enter after editing keys.json..."
}
